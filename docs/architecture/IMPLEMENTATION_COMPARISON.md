...
1. **Worker JavaScript Code**:
   ...
   function processHtmlForPersona(html, persona) {
     ...
     const regex = new RegExp(`<div data-persona="${persona}">([\\s\\S]*?)<\/div>`, 'g');
     let processedHtml = html;

     // Remove all persona containers
     processedHtml = processedHtml.replace(/<div data-persona="[^"]*">[\s\S]*?<\/div>/g, '');
     ...
     matches.forEach(match => {
       ...
       processedHtml = processedHtml.replace('<!-- PERSONA_CONTENT_MARKER -->', match[1]);
     });
...
6. WordPress renders page with ALL persona variations in it:
   ...
   <div data-persona="luxe">Premium cruise description with luxury details...</div>
