	...
	const PersonaFrontend = {
		...
		setupEventHandlers() {
			...
			$(document).on(
				'click',
				'.cme-persona-button',
				this.handlePersonaButtonClick.bind(this)
			);
			...
			$(document).on(
				'change',
				'.cme-persona-select',
				this.handlePersonaDropdownChange.bind(this)
			);
		},
		...
		initDynamicContent() {
			...
			$(
				`.cme-persona-button[data-persona="${this.currentPersona}"]`
			).addClass('active');
			...
			$.ajax({
				...
				success: (response) => {
					if (response.success) {
						...
						// Add active class to the current persona's button
						$(
							`.cme-persona-button[data-persona="${persona}"]`
						).addClass('active');
						...
						if (
							window.cmePersonas &&
							window.cmePersonas.reloadOnSwitch
						) {
				...
				error: () => {
					// Show error message
					this.showMessage(
						'An error occurred while switching personas',
						'error'
					);
		...
		showMessage(message, type) {
			...
			$message
				.html(message)
				.attr('class', `cme-persona-message ${type}`)
				.fadeIn();
